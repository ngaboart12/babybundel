...
import { decrementQuantiy, incrementQuantity, removeFromCart } from '../redux/Slices/cartSlice'
...
const CartScreen = ({navigation}) => {
  ...
  return (
    <View style={{width:'100%',gap:20,height:'100%'}}>
      ...
      <View style={{gap:30}}>
       ...
       <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between'}}>
         <View style={{display:'flex',flexDirection:'row',gap:5}}>
             ...
             <Text style={{fontSize:18,fontWeight:300}}>Select All Product (4)</Text>
         </View>
             <Text>Delete</Text>
         </View>
       ...
       {cart.map((item,index)=>(
         <View key={index} style={{display:'flex',flexDirection:'row',gap:10,marginBottom:20}} >
           <View>
             <Image source={{uri:`${item.image}`}} style={{width:100,height:100}} />
           </View>
           <View style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'space-between',padding:6}}>
             <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between'}}>
             ...
             <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between'}}>
               ...
               <View style={{display:'flex',flexDirection:'row',gap:4}}>
                 <TouchableOpacity onPress={()=>dispatch(decrementQuantiy(item))}>
      ...
      <View style={{display:'flex',flexDirection:'row',width:'100%',alignItems:'center',justifyContent:'center',bottom:0,position:'absolute',left:40}}>
        <View style={{width:'50%',height:60,justifyContent:'center'}}>
          <Text style={{fontFamily:'Red-Hat'}}>Total Price</Text>
          <Text style={{fontSize:20,fontFamily:'Red-Hat'}}>${calculateTotalAmount()}</Text>
        </View>
        <View style={{width:'50%',height:60}}>

        <TouchableOpacity style={{width:'100%',backgroundColor:'#FFB648',height:60,justifyContent:'center'}}>
          <Text style={{textAlign:'center',color:'white',fontSize:16}}>Chek it Out</Text>
        </TouchableOpacity>
        </View>
